...
static const char PAGE_HTML[] PROGMEM =
  ...
  "<div class=spTabs>"
  "<button id=spTabSearchBtn class=active onclick=spSwitchTab('search')>Поиск</button>"
  "<button id=spTabLibBtn onclick=spSwitchTab('lib')>Мои треки</button>"
  "<button id=spTabAlbumsBtn onclick=spSwitchTab('albums')>Мои альбомы</button>"
  "<button id=spTabPlaylistsBtn onclick=spSwitchTab('playlists')>Мои плейлисты</button>"
  "<button id=spTabRecentBtn onclick=spSwitchTab('recent')>Недавнее</button>"
  "<button id=spTabTopBtn onclick=spSwitchTab('top')>Топ</button>"
  "<button onclick=spLogout() style='margin-left:auto;font-size:.8em'>Выйти</button>"
  "</div>"
  "<div id=spSearchTab>"
  ...
  "<button id=spLibMoreBtn style='display:none;margin-top:6px' onclick=spLoadLibrary(false)>Показать ещё</button>"
  "</div>"
  "<div id=spPlaylistsTab style='display:none'>"
  "<div id=spPlaylistsResults class=spRowList></div>"
  "<button id=spPlaylistsMoreBtn style='display:none;margin-top:6px' onclick=spLoadPlaylists(false)>Показать ещё</button>"
  "</div>"
  "<div id=spAlbumsTab style='display:none'>"
  "<div id=spAlbumsResults class=spRowList></div>"
  "<button id=spAlbumsMoreBtn style='display:none;margin-top:6px' onclick=spLoadAlbums(false)>Показать ещё</button>"
  "</div>"
  "<div id=spRecentTab style='display:none'>"
  "<div id=spRecentResults class=spRowList></div>"
  "<button id=spRecentMoreBtn style='display:none;margin-top:6px' onclick=spLoadRecent(false)>Показать ещё</button>"
  "</div>"
  ...
  "<div class=spSubTabs>"
  "<button id=spTopTracksBtn class=active onclick=\"spSwitchTopType('tracks')\">Треки</button>"
  "<button id=spTopArtistsBtn onclick=\"spSwitchTopType('artists')\">Исполнители</button>"
  "<button id=spTopFollowingBtn onclick=\"spSwitchTopType('following')\">Подписки</button>"
  "<select id=spTopRangeSelect onchange=spTopRangeChanged(this.value) style='margin-left:auto'>"
  "<option value=short_term>4 недели</option>"
  "<option value=medium_term selected>6 месяцев</option>"
  "<option value=long_term>Всё время</option>"
  "</select>"
  "</div>"
  ...
  "<span style='font-size:1.05em;color:#8cf'>Добавить в плейлист</span>"
  "<button onclick=closePlaylistPicker()>Отмена</button></div>"
  "<button class=spPrimaryBtn style='width:100%;margin-bottom:10px' onclick=spCreatePlaylistAndAdd()>"
  ...
  "<button onclick=openQueueView() style='font-size:.7em;padding:3px 8px;flex-shrink:0'>Очередь</button>"
  "</div>"
  "</div>"
  "</div>"
  ...
  "let qBtn=document.createElement('button');qBtn.className='spPillBtn';qBtn.title='В очередь';"
  "qBtn.innerText='+Очередь';"
  "qBtn.onclick=function(e){e.stopPropagation();spAddToQueue(track.uri,qBtn)};"
  "row.appendChild(qBtn);"
  "let plBtn=document.createElement('button');plBtn.className='spPillBtn';plBtn.title='В плейлист';"
  "plBtn.innerText='+Плейлист';"
  ...
  "let t=btn.innerText;btn.innerText='Добавлено';setTimeout(()=>btn.innerText=t,1200)"
  "}catch(e){alert('Не удалось добавить в очередь: '+e.message)}}"
  ...
  "let box=document.getElementById('spPlaylistPickerList');box.innerHTML='<div class=spEmpty>Загрузка…</div>';"
  "try{"
  "let d=await spApi('/me/playlists');let items=(d.items||[]).filter(Boolean);"
  "box.innerHTML='';"
  "if(!items.length){box.innerHTML='<div class=spEmpty>Нет своих плейлистов — создайте новый</div>';return}"
  ...
  "}catch(e){box.innerHTML='<div class=spEmpty>Ошибка: '+e.message+'</div>'}}"
  ...
  "closePlaylistPicker();alert('Добавлено в «'+playlistName+'»')"
  ...
  "}catch(e){alert('Ошибка плеера: '+e.message)}}"
  ...
  "title.innerText='Очередь';header.appendChild(title);"
  ...
  "list.appendChild(spSectionTitle('Сейчас играет'));"
  "list.appendChild(spTrackRow(d.currently_playing,()=>{}))}"
  "let queue=d.queue||[];"
  "if(queue.length){"
  "list.appendChild(spSectionTitle('Далее'));"
  "queue.forEach(t=>list.appendChild(spTrackRow(t,()=>{})))}"
  "if(!d.currently_playing&&!queue.length)list.innerHTML='<div class=spEmpty>Очередь пуста</div>'"
  ...
  "if(!items.length&&spPlOffset===0){box.innerHTML='<div class=spEmpty>Нет плейлистов</div>';return}"
  "items.forEach(p=>box.appendChild(spSimpleRow(p.name,"
  "'Плейлист'+(p.owner&&p.owner.display_name?' · '+p.owner.display_name:''),"
  "spImg(p.images),false,()=>openPlaylistDetail(p))));"
  ...
  "let items=(d.items||[]).filter(Boolean);"
  "if(!items.length&&spAlbOffset===0){box.innerHTML='<div class=spEmpty>Нет сохранённых альбомов</div>';return}"
  ...
  "let items=d.items||[];"
  "if(!items.length&&!spRecentBefore){box.innerHTML='<div class=spEmpty>Пока пусто</div>';return}"
  ...
  "box.innerHTML='<div class=spEmpty>Загрузка…</div>';"
  "try{"
  ...
  "box.innerHTML='';"
  "if(!items.length){box.innerHTML='<div class=spEmpty>Нет подписок</div>';return}"
  "items.forEach(a=>box.appendChild(spSimpleRow(a.name,'Исполнитель',spImg(a.images),true,()=>openArtistDetail(a))));"
  "return}"
  "let d=await spApi('/me/top/'+spTopType+'?'+new URLSearchParams({time_range:spTopRange,limit:'20'}));"
  "let items=d.items||[];"
  "box.innerHTML='';"
  "if(!items.length){box.innerHTML='<div class=spEmpty>Пока недостаточно данных для статистики</div>';return}"
  "if(spTopType==='tracks'){"
