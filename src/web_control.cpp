...
void applyWebPowerState(bool off) {
  ...
  if (webPoweredOff) {
    ...
    // сам поставил на паузу заранее, ещё до выключения. И ТОЛЬКО если реально ещё не играет:
    // pausedByPowerOff не сбрасывается сам, если пользователь между выключением и включением
    // сам перезапустил воспроизведение НЕ через нас (например в приложении Spotify — переключил
    // на другое устройство и обратно на Arylic, нажал play) — тогда автовключение по PLAY:1
    // (см. Mega-репозиторий, loop()) уже само подняло систему, трек уже реально играет, а
    // "возобновляющий" onepause здесь просто переключил бы его обратно на паузу. onepause не
    // имеет отдельной команды "только резюм", поэтому единственная защита — не звать её вообще,
    // если и так уже играет
    if (!arylicTrackIsPlaying()) {
      if (arylicSendPlayerCommand("onepause")) {
        arylicNotifyOnepausePressed();
      }
...
static const char PAGE_HTML[] PROGMEM =
  ...
  //
  // cmd(a) здесь больше НЕ вызывается — раньше startHold() слала команду сразу через cmd(),
  // а сервер (motor_ws.cpp) на "up"/"down" ДОПОЛНИТЕЛЬНО планировал первый повтор — при
  // коротком нажатии внутри Dimmer/Source/EQ/Info (Up/Down там — ОДИН шаг списка, не
  // непрерывное вращение мотора) оба почти всегда успевали сработать до отпускания, вместо
  // одного шага получалось два. Починили на сервере: единственный источник команды —
  // сам обработчик "up"/"down" (шлёт немедленно), motorWsPoll() только повторяет, пока
  ...
  // кадры, скопированные из animations/*_animation.cpp Mega-репозитория). Синхронности между
  // "какой кадр анимации сейчас на Mega" и "какой кадр сейчас в браузере" нет и не нужно —
  // осознанное решение (см. чат) в пользу простоты и нулевой доп. нагрузки на UART
  "const ICON_B64={"
  "0:[\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYAwAADAYAAAYHAD/+DYH//wyHHgEIzgYBGGwDARgsAQGYOAEBmBgBAZgYAwGIDAMBiAYHAQgHjgMMA/4DBAAMBgYAGAYDABgMAYAwGADAMDAAcGBgADzhwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYAwAADAcAAAYHADAGDYH//wzHHgEIzgYBGGwDARgsAQGYOAEBmBgBAZgYAwGIDAMBiAYHAQgHjgMMA/wDBAAMBgYAGAYDABgMAYAwGADAYDAAcGBgADzBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYAwAADAcAAAYHgAACDIH//wzHHv8IbgYBGGwDARg8AQGYOAEBmBgBAZgIAwGIDAMBiAYHAQgHjgMMA/wDBAAMBgYAGAYDADAMAYBwGADAYDAAcMBgAD3BwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYA4AADAaAAAYGwAACDEH/gwxnH/8ILgYfGCwDARg8AQGYGAEBmBgBAZgIAwGIDAMBiAYHAQgHjgMMA/wDBAAYBgYAMAYDAHAMAYBgGADAwDAAcYBgAD8BwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABwAAYA0AADAZgAAYGIAACDCH+AwwnH8EIPgf9GBwDHxgcAQGYGAEBmBgBAZgIAwGIDAMBiAYHAQgHjgMMA/wDBAA4BgYAcAYDAOAMAYHAGADDgDAAdwBgAD4BwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAHAA4ADgADABsAAYAzAADAYwAAYGEAACDBH8AwwXHwEIHgfhGBwD+RgcAR+YGAEHmAgBAZgIAwGIDAMBiAYHAQgHjgMMA/wDBABwBgYA4AYDAcAMAYMAGADOADAAfABgADwBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAPAPAAHgA4ADIADABiAAYAwgADAYIAAYGCAACDAn8AwwPHgEIDgeBGAwDwRgcAXGYCAE9mAgBD5gIAweIDAMBiAYHAQgHjgMMA/gDBABwBgYBwAYDBwAMAZwAGADwADAAcABgADwBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOgPAAGIA4ADGADABhgAYAwYADAYEAAYGBAACDA38AwwPHgEIDgcBGAwDgRgMAcGYCAFhmAgBMZgIAxmIDAMPiAYHBwgHjgMMA/gDBAHgBgYPAAYDeAAMAcAAGADAADAAcABgADwBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAA
YAAAYDAAAMAYAAGADAADAAcABgADwBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYAwAADAYAAAYGADgCDAA4AwwAOAEIAAABGABAARgA4AGYAGABmABgAZgA4AGIAOABiADAAQgBwAMMA8ADBAHgBgYAAAYDAAAMAYAAGADAADAAcABgADwBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYAwAADAYAEAYGAHgCDAA4AwwAMAEIAAABGABAARgA8AGYAGABmABgAZgA4AGIAOABiADgAQgAwAMMAcADBAHgBgYAAAYDAAAMAYAAGADAADAAcABgADwBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYAwAADAYAcAYGAHACDABwAwwAAAEIAAABGADAARgA4AGYAGABmABgAZgAYAGIAOABiADgAQgA4AMMAOADBAHgBgYA8AYDAAAMAYAAGADAADAAcABgADwBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYAwAADAYAcAYGAHACDABwAwwAAAEIAAABGADgARgA4AGYAGABmABgAZgAYAGIAGABiADgAQgA4AMMAOADBADwBgYB8AYDAAAMAYAAGADAADAAcABgADwBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYAwAgDAYA8AYGAPACDABwAwwAAAEIAAABGADgARgA4AGYAGABmABgAZgAYAGIAGABiABgAQgAYAMMAGADBADwBgYB8AYDAAAMAYAAGADAADAAcABgADwBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYAwBgDAYA8AYGAPACDABgAwwAAAEIAAABGADgARgA4AGYAGABmABgAZgAYAGIAGABiABgAQgAYAMMAGADBABgBgYB+AYDAAAMAYAAGADAADAAcABgADwBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYAwBgDAYA8AYGAPACDABgAwwAAAEIAAABGADgARgA4AGYAGABmABgAZgAYAGIAGABiABgAQgAYAMMAGADBABgBgYB+AYDAAAMAYAAGADAADAAcABgADwBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYAwBgDAYA8AYGAPACDABgAwwAAAEIAAABGADgARgA4AGYAGABmABgAZgAYAGIAGABiABgAQgAYAMMAGADBABgBgYB+AYDAAAMAYAAGADAADAAcABgADwBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYAwBgDAYA8AYGAPACDABgAwwAAAEIAAABGADgARgA4AGYAGABmABgAZgAYAGIAGABiABgAQgAYAMMAGADBABgBgYB+AYDAAAMAYAAGADAADAAcABgADwBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYAwBgDAYA8AYGAPACDABgAwwAAAEIAAABGADgARgA4AGYAGABmABgAZgAYAGIAGABiABgAQgAYAMMAGADBABgBgYB+AYDAAAMAYAAGADAADAAcABgADwBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYAwBgDAYA8AYGAPACDABgAwwAAAEIAAABGADgARgA4AGYAGABmABgAZgAYAGIAGABiABgAQgAYAMMAGADBABgBgYB+AYDAAAMAYAAGADAADAAcABgADwBwAAP/wAAAfAAAAAAAA=\",\"AAAAAAAD4AAAP/wAAOAPAAGAA4ADAADABgAAYAwBgDAYA8AYGAPACDABgAwwAAAEIAAABGADgARgA4AGYAGABmABgAZgAYAGIAGABiABgAQgAYAMMAGADBABgBgYB+AYDAAAMAYAAGADAADAAcABgADwBwAAP/wAAAfAAAAAAAA=\"]"
  "};"
  "const ICON_POS={0:[0,10],1:[0,10],2:[0,10],3:[0,10],4:[0,10],5:[0,10],6:[5,10],7:[0,10]};"
  "const ICON_FRAME_DELAY_MS=42;"
  // Раскодируем base64 в байты один раз при загрузке страницы (не на каждый тик анимации) —
  // ICON_FRAMES[iconId] — массив Uint8Array(128), один на кадр, тот же тайловый XBM-порядок
  // (MSB первый — бит 7 самый левый пиксель), что и в исходных .cpp на Mega
  "function b64ToBytes(s){"
  "let bin=atob(s);let out=new Uint8Array(bin.length);"
  "for(let i=0;i<bin.length;i++)out[i]=bin.charCodeAt(i);"
  "return out"
  "}"
  "let ICON_FRAMES={};"
  "for(let k in ICON_B64){ICON_FRAMES[k]=ICON_B64[k].map(b64ToBytes)}"
  "let currentIconId=255;"
  "let iconAnimFrame=0;"
  // Рисует один кадр набора iconId поверх уже нарисованного зеркала — 32x32, MSB-first
  // (бит 7 — самый левый пиксель байта, как в исходном Adafruit GFX/XBM-экспорте на Mega)
  "function drawIconOverlay(){"
  "if(!megaFrameCtx||currentIconId===255||!ICON_FRAMES[currentIconId])return;"
  "let frames=ICON_FRAMES[currentIconId];"
  "let frame=frames[iconAnimFrame%frames.length];"
  "let pos=ICON_POS[currentIconId];"
  "let ox=pos[0],oy=pos[1];"
  "let img=megaFrameCtx.createImageData(32,32);"
  "for(let y=0;y<32;y++){"
  "for(let x=0;x<32;x++){"
  ...
  "}"
  // Свой независимый таймер — крутится всё время, пока currentIconId!=255, вообще не завися
  // от того, когда реально пришёл следующий кадр зеркала по UART/WebSocket
  "setInterval(function(){"
  "if(currentIconId===255)return;"
  "iconAnimFrame++;"
  "drawIconOverlay()"
  "},ICON_FRAME_DELAY_MS);"
  ...
  "}"
  "let pendingIconId=255,pendingScreenId=0;"
  "function restoreAfterUnmute(){"
  "currentIconId=pendingIconId;"
  "currentScreenId=pendingScreenId;"
  ...
  "let newIconId=bytes[1024];"
  "let newScreenId=bytes[1025];"
  // Пока играет переход из Mute — НЕ показываем новый кадр сразу (иначе анимация оказалась бы
  // поверх уже показанного нового экрана вместо "чистого" перехода, как жаловался пользователь).
